...
public class PlayerShooting : MonoBehaviour
{
    ...
    //This is for vibrating xbox gamepad P1
    ...
    void Update ()
    {
        ...
        if (Input.GetAxisRaw("Fire1") == 0 && isShooting)
        {
            ...
            GamePad.SetVibration(playerIndex, 0.0f, Input.GetAxisRaw("Fire1") / 2);


        //Checks if timebetweenbullets has passed and fire1 is pressed
        //Input getaxisraw is for xbox gamepad
        if ((Input.GetButton ("Fire1") || Input.GetAxisRaw("Fire1") > 0) && timer >= timeBetweenBullets /*&& Time.timeScale != 0*/)
        {
            Shoot ();
        }
